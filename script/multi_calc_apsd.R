multi_calc_apsd=function(data, FR_name, nrow_throat, presep=FALSE, type="citdas", impactor="aci"){
  ...
  ####
  df.nest=data %>% pivot_longer(-1) %>%
    pivot_wider(names_from=1, values_from = "value") %>% 
    pivot_longer(-all_of(1:nrow_throat), 
                 names_to = ".stages", values_to = ".mass") %>% 
    mutate(across(all_of(c(FR_name,".mass")), as.numeric)) %>% 
    select( all_of(c("name", FR_name, ".stages",".mass")) ) %>% 
    df_nest_by(vars = c("name", FR_name))

  apsd.result=df.nest %>% 
    mutate(
    .restults=map2(data, .data[[FR_name]],
              ~apsd_calc(.x,".stages",".mass",FR=.y, presep=presep, type=type, impactor=impactor))) %>% 
          unnest(.restults) %>% 
    select(-data)
  ...
  apsd.df=data %>% 
    ...
    pivot_wider(names_from = 1, values_from = value) %>% 
    left_join(apsd.result %>% select(!contains(FR_name)), by="name")
